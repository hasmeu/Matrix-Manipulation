...
struct Matrix add(struct Matrix matrix1, struct Matrix matrix2) {
    //checks if the matrices can be added.
    ...
    //creates the matrix that will be returned in this function.
    struct Matrix result;
    result.type = matrix1.type;
    result.rows = matrix1.rows;
    result.cols = matrix1.cols;
    
    // Allocate memory for result.data. array of pointers of DataValue type that points to a number of type DataValue.
    ...
    for (int i = 0; i < matrix1.rows; i++) {
        //for each row, the data at [i][j] will be added and placed into the matrix that was created.
...
//this function is the same as add, but it subtracts each data point instead. 
...
struct Matrix multiply(struct Matrix matrix1, struct Matrix matrix2) {
    ...
    //iterates over the rows of m1 and the cols of m2
    for (int i = 0; i < matrix1.rows; i++) {
        for (int j = 0; j < matrix2.cols; j++) {
            switch (matrix1.type) {
                case INT:
                    result.data[i][j].int_val = 0;
                    //iterates over the cols of m1
                    //ex: at [0][0], 1x1, + 2x3 = 7
...
//create possible type conversion method
